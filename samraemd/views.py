                            ...
                            if student:
                                ...
                                if 'skp' in exam_code:
                                    results_dict = {
                                        ...
                                        'al_re'        : str(sheet.cell_value(row, 1)).strip(),
                                        'hp_re'        : str(sheet.cell_value(row, 2)).strip(),
                                        'ra_re'        : str(sheet.cell_value(row, 3)).strip(),
                                        'rm_re'        : str(sheet.cell_value(row, 4)).strip(),
                                        're'           : str(sheet.cell_value(row, 5)).strip(),
                                        'al_sg'        : str(sheet.cell_value(row, 6)).strip(),
                                        'hp_sg'        : str(sheet.cell_value(row, 7)).strip(),
                                        'ra_sg'        : str(sheet.cell_value(row, 8)).strip(),
                                        'rm_sg'        : str(sheet.cell_value(row, 9)).strip(),
                                        'he'           : str(sheet.cell_value(row, 10)).strip(),
                                        'sg'           : str(sheet.cell_value(row, 11)).strip(),
                                        'fm'           : str(sheet.cell_value(row, 12)).strip(),
                        ...
                        for row in range(1, sheet.nrows):
                            ...
                            if student:
                                ...
                                if 'skp' in exam_code:
                                    results_dict = {
                                        'student'     : student.first(),
                                        'le_re'       : str(sheet.cell_value(row, 1)).strip(),
                                        'mn_re'       : str(sheet.cell_value(row, 2)).strip(),
                                        're'          : str(sheet.cell_value(row, 3)).strip(),
                                        'le_sg'       : str(sheet.cell_value(row, 4)).strip(),
                                        'mn_sg'       : str(sheet.cell_value(row, 5)).strip(),
                                        'sg'          : str(sheet.cell_value(row, 6)).strip(),
                                        'he'          : str(sheet.cell_value(row, 7)).strip(),
                                        'fm'          : str(sheet.cell_value(row, 8)).strip(),
                                        'fm_fl'       : str(sheet.cell_value(row, 9)).strip(),
                                        'exam_code'   : exam_code,
                                        'exam_date'   : exam_date,
                                        'student_year': student_year
                                    }
...
class SamraemdENSResultAdminListing(cm_mixins.SuperUserMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['exams']     = s_models.SamraemdENSResult.objects.all().values(
            'exam_date', 'student_year', 'exam_code').distinct()
        context['raw_exams'] = s_models.SamraemdResult.objects.filter(exam_code=2).values(
            'exam_date', 'student_year', 'exam_name', 'exam_code').distinct()
        return context
                    ...
                    for sheetsnumber in range(book.nsheets):
                        ...
                        for row in range(1, sheet.nrows):
                            ...
                            if student:
                                ...
                                results_dict = {
                                    'student'     : student.first(),
                                    'le_re'       : str(int(sheet.cell_value(row, 1))).strip(),
                                    'mn_re'       : str(int(sheet.cell_value(row, 2))).strip(),
                                    're'          : str(int(sheet.cell_value(row, 3))).strip(),
                                    'le_sg'       : str(int(sheet.cell_value(row, 4))).strip(),
                                    'mn_sg'       : str(int(sheet.cell_value(row, 5))).strip(),
                                    'sg'          : str(int(sheet.cell_value(row, 6))).strip(),
                                    'he'          : str(int(sheet.cell_value(row, 7))).strip(),
                                    'fm'          : str(sheet.cell_value(row, 8)).strip(),
                                    'exam_code'   : exam_code,
                                    'exam_date'   : exam_date,
                                    'student_year': student_year
